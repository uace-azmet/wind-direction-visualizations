#' `fxnFigureFacets`: generates polar graphs of summarized wind direction data by month
#' 
#' @param: `inData` - data table of daily AZMet station data
#' @return: `figureFacets` - ggplot polar graph of summarized wind direction data
#' 
fxnFigureFacets <- function(inData) {
  
  figureFacets <- ggplot2::ggplot(data = inData, mapping = aes(x = wind_vector_dir)) +  
    ...
    coord_polar() +
    facet_wrap(~ lubridate::month(inData$datetime, label = TRUE), ncol = 3) +
